...
def _reference_viewpoints():
    """
    Build the list of reference camera poses sampled at startup.

    Poses are derived from the configured starting position with a spread of
    yaw / pitch values so the learned features cover the terrain from several
    arbitrary directions. Configurable via CONFIG keys (all optional):
        feature_ref_yaws    : list of yaw angles (deg)
        feature_ref_pitches : list of pitch angles (deg)
    ...
    m  = CONFIG.get("margin")
    base_x = -iw / m / 2
    base_y = float(CONFIG.get("start_h"))
    base_z = -(ih / m) - 100

    yaws    = CONFIG.get("feature_ref_yaws",    [-40, -20, 0, 20, 40])
    pitches = CONFIG.get("feature_ref_pitches", [20, 35])

    poses = []
    for pitch in pitches:
        for yaw in yaws:
            # Slight lateral offset with yaw so views aren't all co-located.
            off = math.sin(math.radians(yaw)) * 60.0
            poses.append((base_x + off, base_y, base_z, float(pitch),
                          float(yaw), 0.0))
    return poses
...
def render_scene(apply_input=True, recording_mode=True, trackers_mode=False,
    ...
    if apply_input and (not picking_mode or trackers_mode or feature_mode):
        ...
        if recording_mode or trackers_mode or feature_mode:
            ...
            if keys_pressed[K_DOWN]:   r_x += rot_speed;  moved = True
            if keys_pressed[K_UP]:     r_x -= rot_speed;  moved = True
...
def draw(recording_mode, trackers_mode=False, feature_mode=False):
    ...
    if (feature_mode and feature_overlay_active and feature_cam_pairs
        ...
        f_pos_err = math.sqrt((est_cam[0] - real_cam[0]) ** 2 +
                              (est_cam[1] - real_cam[1]) ** 2 +
                              (est_cam[2] - real_cam[2]) ** 2)
        f_rot_raw = (min(abs(est_cam[3] + real_cam[3]), 360 - abs(est_cam[3] + real_cam[3])) +
                     min(abs(est_cam[4] + real_cam[4]), 360 - abs(est_cam[4] + real_cam[4])) +
                     min(abs(est_cam[5] + real_cam[5]), 360 - abs(est_cam[5] + real_cam[5])))
        f_rot_err = min(f_rot_raw % 360, 360 - f_rot_raw % 360)
