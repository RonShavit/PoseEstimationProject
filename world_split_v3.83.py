...
def draw(recording_mode, trackers_mode=False):
    ...
    if trackers_mode and tracker_overlay_active and tracker_cam_pairs:
        ...
        print(f"[overlay] idx={tracker_current_pair_index} "
              f"real={tuple(round(v,1) for v in real_cam)} "
              f"est=({ex:.1f},{ey:.1f},{ez:.1f},{erx:.1f},{ery:.1f},{erz:.1f})")
        ...
        # draw_terrain_vbo (the base pass) left GL_COLOR_ARRAY enabled; with it
        # on, glColor4f is ignored and stale per-vertex colours are used. Turn
        # it off so the flat tint below actually applies.
        glDisableClientState(GL_COLOR_ARRAY)
        glEnableClientState(GL_VERTEX_ARRAY)
        glVertexPointer(3, GL_FLOAT, stride, ctypes.c_void_p(3 * 4))
        glColor4f(1.0, 0.0, 0.0, 1.0)   # DIAGNOSTIC: opaque red, dial back later
