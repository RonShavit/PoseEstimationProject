...
def render_scene(apply_input=True, recording_mode=True):
    ...
    global picking_mode, trackers_mode
...
def save_left_screenshot():
    """
    Capture the left viewport from the OpenGL framebuffer and save as
    screenshot.png.  Uses glReadPixels so it works with OpenGL rendering.
    """
    width, height = pygame.display.get_surface().get_size()
    half_w = width // 2

    # Read pixels from the left half (OpenGL origin is bottom-left)
    ...
    # Flip vertically (OpenGL bottom-left → image top-left) and RGB → BGR for cv2
    img = np.flipud(img)
...
def draw(recording_mode):
    global picking_mode, trackers_mode, pnp_result, picked_correspondences
    global c_x, c_y, c_z, r_x, r_y, r_z
    global c_x2, c_y2, c_z2, r_x2, r_y2, r_z2
        ...
        for event in pygame.event.get():
            ...
            if event.type == KEYDOWN:
                ...
                if event.key == K_t:
                    ...
                    if trackers_mode:
                        # Entering trackers mode — exit other modes cleanly
                        picking_mode   = False
                        pnp_result     = None
                ...
                if event.key == K_r:
                    ...
                    if recording_mode:
                        trackers_mode = False   # can't replay and track simultaneously
                    print(f"Recording mode: {recording_mode}")
                if event.key == K_p:
                    picking_mode = not picking_mode
                    if picking_mode:
                        trackers_mode = False   # mutually exclusive
