...
def _look_at_pose(cam_world, target_world):
    ...
    -cos pitch cos yaw), which inverts (for the forward-facing hemisphere) to:
        yaw   = asin(f.x)
        pitch = atan2(-f.y, -f.z)
...
def _reference_viewpoints():
    ...
    CONFIG keys (all optional):
        ...
        feature_ref_seed         : RNG seed for reproducibility (default 0)
    ...
    seed       = int(CONFIG.get("feature_ref_seed", 0))
    ...
    for _ in range(n):
        ...
        # Height above the terrain top, scaled to the world size.
