            ...
            if event.type == KEYDOWN:
                ...
                if event.key == K_b:
                    if trackers_mode:
                        ...
                        # TODO: feature-matching pose estimation.
                        #   1. Detect & match features between this left-view
                        #      screenshot and a reference image / the map.
                        #   2. Recover 2D-3D correspondences from the matches.
                        #   3. Run PnP to estimate (cam_pos, euler, R, tvec).
                        # Until implemented, fall back to the actual pose so the
                        # framework (overlay + error readout) is exercisable with
                        # zero error.
                        feat_result = None   # replace with the estimator output
                        if feat_result is not None:
                            est_cam_pos, est_euler, est_R, est_tvec = feat_result
                        else:
                            # Placeholder identity estimate = actual left camera.
