...
def draw_tracker_sphere(x, y, z):
    """Draw a green sphere to mark a tracker point on the left view."""
    glPushMatrix()
    glTranslatef(x, y, z)
    glColor3f(0.0, 1.0, 0.2)
    gluSphere(get_sphere_quadric(), 0.35, 16, 8)
    glPopMatrix()
...
def render_scene(apply_input=True, recording_mode=True, trackers_mode=False):
    ...
    # Trackers mode: draw tracker markers on the left (apply_input) view
    if apply_input and trackers_mode:
        for pos in tracker_points:
            px, py, pz = pos
            glPushMatrix()
            glLoadIdentity()
            glRotatef(r_y, 0, 1, 0); glRotatef(r_x, 1, 0, 0)
            glRotatef(r_z, 0, 0, 1); glTranslatef(c_x, c_y, c_z)
            draw_tracker_sphere(px, py, pz)
            glPopMatrix()
