...
def draw_tracker_sphere(x, y, z, r, g, b):
    """Draw a tracker sphere  with the given RGB colour (0-255)."""
    glPushMatrix()
    glTranslatef(x, y, z)
...
def render_scene(apply_input=True, recording_mode=True):
    ...
    global picking_mode, trackers_mode, trackers_list
    ...
    # Draw tracker spheres in the left (free-move) view when in trackers mode
    if apply_input and trackers_mode:
        for tx, ty, tz, tr, tg, tb in trackers_list:
            draw_tracker_sphere(tx, ty, tz, tr, tg, tb)
...
def main():
    ...
    picking_mode           = False
    trackers_mode          = False
    trackers_list          = get_trackers_from_file("trackers.trk")
