...
def render_scene(apply_input=True, recording_mode=True, trackers_mode=False):
    ...
    if apply_input and (not picking_mode or trackers_mode):
        keys_pressed = pygame.key.get_pressed()
        if recording_mode or trackers_mode:
            moved = False
...
# ---------------------------------------------------------------------------
def draw(recording_mode, trackers_mode=False):
    ...
    if trackers_mode and tracker_overlay_active and tracker_cam_pairs:
        ...
        glEnable(GL_BLEND)
        # Per-vertex terrain colours have no alpha, so fade the whole pass with
        # a constant blend factor (GL 1.4+, the live render context supports it).
        glBlendColor(0.0, 0.0, 0.0, 0.36)
        glBlendFunc(GL_CONSTANT_ALPHA, GL_ONE_MINUS_CONSTANT_ALPHA)

        # The PnP-estimated Euler angles are stored in the negated convention
        # (the error metrics compare euler vs -actual, and the green pyramid is
        # drawn with glRotatef(-ery,...)). The live left camera applies
        # Rot(r_y)·Rot(r_x)·Rot(r_z)·Translate(c) to the ACTUAL angles, so to
        # view from the estimate we feed it the negated estimated angles.
        nry, nrx, nrz = -ery, -erx, -erz
        erx_a = math.cos(nry * math.pi / 180)
        erz_a = math.sin(nry * math.pi / 180)
        glLoadIdentity()
        glRotatef(nry, 0,     1, 0)
        glRotatef(nrx, erx_a, 0, erz_a)
        glRotatef(nrz, erz_a, 0, erx_a)
        glTranslatef(ex, ey, ez)
        draw_terrain_vbo(terrain_vbo, terrain_vertex_count)
